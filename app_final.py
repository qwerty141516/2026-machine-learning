...
if clicked is not None:
    ...
    # 🛠 [신규 추가] 버튼식(Expander) 추천 점수 산출 근거 확인 영역
    with st.expander("🔍 AI 추천 점수 산출 근거 확인하기"):
        st.markdown(f"### 📋 점수 세부 진단 내역 (최종 점수: **{score}점**)")
        st.write(f"- **기본 평가 점수**: `85점` (거시경제 기본 모델 기준)")
        
        # 가격 차이에 따른 세부 수치 브리핑
        if price_gap > 0:
            st.write(f"- ❌ **가격 고평가 감점**: 현재 매물가({clicked['rent']}만원)가 ML 예측 적정가({pred:.1f}만원)보다 **{price_gap:.1f}만원** 더 높습니다. 이에 따라 추천도가 약 **{price_gap*2.0:.1f}점 감점**되었습니다.")
        else:
            st.write(f"-  **가격 가성비 가점**: 현재 매물가({clicked['rent']}만원)가 ML 예측 적정가({pred:.1f}만원)보다 **{abs(price_gap):.1f}만원** 더 저렴하여 우수한 가성비를 보입니다. 이에 따라 추천도가 약 **{abs(price_gap)*2.0:.1f}점 가점**되었습니다.")
            
        # 매물 물리적 요인 부가 설명
        if clicked['walk'] <= 5:
            st.write("- ⭐ **입지 가점**: 학교까지 도보 거리({clicked['walk']}분)가 매우 가까워 가산 요소로 작용했습니다.")
        elif clicked['walk'] >= 12:
            st.write("- 🏃 **입지 감점**: 학교까지 도보 거리({clicked['walk']}분)가 다소 멀어 접근성이 감점 요인으로 고려되었습니다.")
            
        st.caption("※ 본 점수는 거시경제 변수(금리·환율·수급지수) 및 원룸 내부 조건(방 크기·보증금 구조)을 랜덤 포레스트 알고리즘으로 종합 교차 분석한 가성비 스코어입니다.")
    ...
    with tab3:
        ...
        **2. 계약 당일 필수 액션 (대항력 갖추기)**
        ...
        st.warning("""
        **3. 중개수수료(복비) 바가지 예방**
        - 중개수수료는 법정 상한 요율이 정해져 있습니다 (보통 대학가 원룸 요율은 거래금액 of 0.4%~0.5% 내외).
